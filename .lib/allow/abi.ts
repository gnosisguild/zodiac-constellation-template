...
export const ABIS_DIR = path.resolve(__dirname, "..", "..", "abis");
...
export const abiFilePath = (node: ContractNode): string =>
  path.join(ABIS_DIR, node.chain, ...node.segments) + ".json";

export const readAbi = (node: ContractNode): Abi | null => {
...
export const writeAbi = (node: ContractNode, abi: Abi): void => {
  const file = abiFilePath(node);

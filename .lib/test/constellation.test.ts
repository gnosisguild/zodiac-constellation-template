...
describe("constellation API", () => {
  ...
  describe("existing safe — bracket access", () => {
    ...
    it("returns a node ref with existing properties merged with overrides", () => {
      ...
      expect(ggDao.address).toBe(codegen.vaults["GG DAO"].address);
      expect(ggDao.vault).toBe(true); // overridden from false
      expect(ggDao.threshold).toBe(codegen.vaults["GG DAO"].threshold);
    ...
    it("returns a node ref without overrides via empty call", () => {
      ...
      expect(treasury.address).toBe(codegen.vaults["Treasury"].address);
    });
  ...
  describe("existing roles — bracket access", () => {
    it("returns canonical roles mod with config applied", () => {
      ...
      expect(ggDaoRoles.target).toBe(ZODIAC_ROLES["GG DAO"].target);
      expect(ggDaoRoles.owner).toBe(ZODIAC_ROLES["GG DAO"].owner);
      expect(ggDaoRoles.avatar).toBe(ZODIAC_ROLES["GG DAO"].avatar);
  ...
  describe("policies accessor", () => {
    it("returns a policy ref for an existing policy", () => {
      const eth = constellation({
        workspace: "w",
        label: "l",
        chain: 1,
      ...
      const policy = eth.policies["My existing policy"]({});

      expect(policy.label).toBe("My existing policy");
